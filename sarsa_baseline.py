...
for k in range(K):
    ...
    for h in range(H):
        a = sarsa.choose_action(str([fake_s[0],fake_s[1],fake_s[2]]))
        ...
        if fake_s_[1] == 0:
            fake_s_ = "terminal"
        sarsa.learn(str([fake_s[0],fake_s[1],fake_s[2]]),a,rk_norm,str([fake_s_[0],fake_s_[1],fake_s_[2]]), a_)
        # update s
        if fake_s_[1] == 0:
            break
        true_s = true_s_
...
plt.savefig("fig-sarsa-02-03-20-08.png")
with open("log-sarsa-02-03-20-08.csv", "w") as csvfile:
